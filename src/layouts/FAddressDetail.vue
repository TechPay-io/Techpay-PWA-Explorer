                    ...
                    <div class="balance center-v">
                        <h3 class="h1">
                            <span v-show="cAccount">
                                <t-p-c-token-value :value="cAccount ? cAccount.totalValue : 1" convert no-currency /> <span class="tpc">TPC</span>
                            </span>
                        </h3>
                        <div v-show="cAccount" class="usd">
                            <t-p-c-token-value :value="toUSD(cAccount ? cAccount.totalValue : 1)" with-price-currency no-currency />
                        </div>
                    </div>
                </f-card>
            </div>
                    ...
                    <div class="balance center-v">
                        <h3 class="h1">
                            <span v-show="'available' in cAssets">
                                <t-p-c-token-value :value="cAssets.available" convert no-currency /> <span class="tpc">TPC</span>
                            </span>
                        </h3>
                        <div v-show="'available' in cAssets" class="usd">
                            <t-p-c-token-value :value="toUSD(cAssets.available)" with-price-currency no-currency />
                        </div>
                    </div>
                </f-card>
            </div>
                ...
                <f-card>
                    ...
                    <!--
                                            <div class="row no-collapse">
                                                ...
                                                <div class="col">
                                                    <div v-show="'available' in cAssets">{{ toTPC(cAssets.available) }} TPC</div>
                                                </div>
                                            </div>
                    -->
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <div v-show="'delegated' in cAssets"><t-p-c-token-value :value="cAssets.delegated" /></div>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        <div class="col f-row-label">{{ $t('view_address_detail.pending_rewards') }}</div>
                        <div class="col">
                            <div v-show="'pending_rewards' in cAssets"><t-p-c-token-value :value="cAssets.pending_rewards" /></div>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        <div class="col f-row-label">{{ $t('view_address_detail.stashed_rewards') }}</div>
                        <div class="col">
                            <div v-show="'stashed' in cAssets"><t-p-c-token-value :value="cAssets.stashed" convert /></div>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <div v-show="'claimed_rewards' in cAssets">
                                -
                                <!--{{ toTPC(cAssets.claimed_rewards, true) }} TPC-->
                            </div>
                        </div>
                    </div>
                    <div class="row no-collapse">
        ...
        <!--
                    ...
                    <f-card>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="num-block">
                                    <h2 class="h3">{{ $t('view_address_detail.value_in_tpc') }}</h2>
                                    <div class="num"><span v-show="cAccount">{{ toTPC(cAccount ? cAccount.totalValue : 1) }}</span></div>
                                </div>
                            </div>
                            <div class="col">
                                <div class="num-block">
...
<script>
    ...
    import { WEIToTPC, TPCToUSD } from "../utils/transactions.js";
...
/*
                dAssetColumns: [
                    ...
                    {
                        name: 'valueInTPC',
                        label: this.$t('view_address_detail.value_in_tpc'),
                        cssClass: 'align-end',
                    },
                    {
                        name: 'valueInUSD',
                ...
                if (cAccount) {
                    ...
                    if (delegations && delegations.edges) {
                        delegations.edges.forEach((_edge) => {
                            ...
                            assets.delegated += (delegation ? WEIToTPC(delegation.amount) : 0);
                            assets.pending_rewards += (delegation && delegation.pendingRewards ? WEIToTPC(delegation.pendingRewards.amount) : 0);
                            assets.claimed_rewards += (delegation ? WEIToTPC(delegation.claimedReward) : 0);
                        });
...
/*
            getAssetItem(_assetName, _value) {
                return {
                    asset: _assetName,
                    balance: this.toTPC(_value),
                    valueInTPC: this.toTPC(_value),
...
*/
             ...
             * @return {string}
             */
            toTPC(_value, _isNumber) {
                return _isNumber ? _value : WEIToTPC(_value);
            },
             ...
             */
            toUSD(_value) {
                return TPCToUSD(WEIToTPC(_value), this.$store.state.tokenPrice);
            },
