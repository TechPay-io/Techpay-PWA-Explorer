...
-->
                ...
                <!--
                            <template v-slot:column-fee="{ value, column }">
                                <div v-if="column" class="row no-collapse no-vert-col-padding">
                                    <div class="col-5 f-row-label">{{ column.label }}</div>
                                    <div class="col">{{ WEIToTPC(value | formatHexToInt) }}</div>
                                </div>
                                <template v-else>
                                    {{ WEIToTPC(value | formatHexToInt) }}
                                </template>
                            </template>
                -->
...
*/
                    ...
                    {
                        name: 'amount',
                        label: `${this.$t('view_transaction_list.amount')} (TPC)`,
                        itemProp: `${!this.withoutCursor ? 'transaction.' : ''}value`,
                        formatter: _value => {
                            // return formatNumberByLocale(numToFixed(WEIToTPC(_value), 2), 2)
...
/*
                    {
                        name: 'fee',
                        label: `${this.$t('view_transaction_list.fee')} (TPC)`,
                        itemProp: 'gasUsed',
                        width: '130px'
                    }
*/
